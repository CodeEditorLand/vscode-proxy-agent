...
export function createProxyResolver(params: ProxyAgentParams) {
	...
	function resolveProxyWithRequest(
		flags: { useProxySettings: boolean; addCertificatesV1: boolean },
		...
		addCertificatesToOptionsV1(
			params,
			flags.addCertificatesV1,
			opts,
			() => {
...
export type ResolveProxyWithRequest = (
	flags: { useProxySettings: boolean; addCertificatesV1: boolean },
	req: http.ClientRequest,
	opts: http.RequestOptions,
	url: string,
	...
	function patch(original: typeof http.get) {
		function patched(
			...
			const useProxySettings =
				...
				(config === "override" ||
					config === "fallback" ||
					(config === "on" && originalAgent === undefined));
			...
			if (useProxySettings || addCertificatesV1) {
				...
				const resolveP = (
					...
					new Promise<string | undefined>((resolve) =>
						resolveProxy(
							{ useProxySettings, addCertificatesV1 },
							req,
							opts,
							url,
							resolve,
				...
				const agent = createPacProxyAgent(
					resolveP,
					{
						originalAgent:
							!useProxySettings ||
						...
						_vscodeTestReplaceCaCerts: (
							options as SecureContextOptionsPatch
						)._vscodeTestReplaceCaCerts,
					},
					(opts) =>
...
export function createNetPatch(
	params: ProxyAgentParams,
...
export function createTlsPatch(
	params: ProxyAgentParams,
	originals: typeof tls,
) {
	return {
		connect: patchTlsConnect(params, originals.connect),
		createSecureContext: patchCreateSecureContext(
			originals.createSecureContext,
		),
	};
}
...
function patchCreateSecureContext(
	original: typeof tls.createSecureContext,
): typeof tls.createSecureContext {
	return function (
...
export function createFetchPatch(
	...
	return async function patchedFetch(
		...
		const proxyAuthorization = await params.lookupProxyAuthorization?.(
			proxyURL,
			undefined,
			state,
		);
		const modifiedInit = {
			...
			dispatcher: new undici.ProxyAgent({
				...
				headers: proxyAuthorization
					...
					: undefined,
				requestTls: requestCA
					? { allowH2, ca: requestCA }
					: { allowH2 },
				proxyTls: proxyCA ? { allowH2, ca: proxyCA } : { allowH2 },
				clientFactory: (origin: URL, opts: object): undici.Dispatcher =>
					(new undici.Pool(origin, opts) as any).compose(
						(dispatch: undici.Dispatcher["dispatch"]) => {
							class ProxyAuthHandler extends undici.DecoratorHandler {
								private abort:
									| ((err?: Error) => void)
									| undefined;
								constructor(
									private dispatch: undici.Dispatcher["dispatch"],
									private options: undici.Dispatcher.DispatchOptions,
									private handler: undici.Dispatcher.DispatchHandlers,
								) {
									super(handler);
								}
									...
									(async () => {
										try {
											const proxyAuthorization =
												await params.lookupProxyAuthorization?.(
													proxyURL!,
...
export function patchUndici(originalUndici: typeof undici) {
	...
	const patchedProxyAgent = function PatchedProxyAgent(
		opts: undici.ProxyAgent.Options | string,
	): undici.ProxyAgent {
...
function getAgentOptions(
	...
	const originalAgentOptions: undici.Agent.Options | undefined =
...
function addCertificatesToOptionsV1(
	params: ProxyAgentParams,
	addCertificatesV1: boolean,
	opts: http.RequestOptions | tls.ConnectionOptions,
	callback: () => void,
) {
	if (addCertificatesV1) {
		getOrLoadAdditionalCertificates(params)
			...
			.catch((err) => {
				params.log.error(
					"ProxyResolver#addCertificatesV1",
					toErrorMessage(err),
				);
			});
	} else {
